...
\author[Author information pending]{Youhan Wanigasuriya$^{1}$\thanks{youhanwanigasuriya@gmail.com}\\
...
The primary pipeline performs no imputation, learned feature selection or principal-component transformation. Because the number of independent systems, particularly BH systems, is limited, inner source-level validation provides a relatively coarse criterion for model selection; the hyperparameter search is therefore intentionally restricted to the frozen small grids described above. Split membership, selected hyperparameters and preprocessing audits are retained in Supplement~S3. The principal random seed is 42.
